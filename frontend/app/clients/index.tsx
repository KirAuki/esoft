...
import { FlatList, View , Text, TextInput ,StyleSheet, Button, Alert} from "react-native";
...
function ClientsList() {
    ...
    async function fetchClients(searchQuery: string = '') {
        setLoading(true);
        try {
            const endpoint = searchQuery ? `${API_BASE_URL}/clients/search/` : `${API_BASE_URL}/clients/`;
        ...
        <View style={styles.container}>
            ...
            renderItem={({ item }) => (
                ...
                <Link href={{ pathname: '/clients/[id]', params: { id: item.id } }}>
                    <View>
                    <Text style={styles.clientName}>{`ФИО: ${item.last_name || ''} ${item.first_name || ''} ${item.patronymic || ''}`}</Text>
            ...
            />
            <Button title="Создать клиента" onPress={() => setModalVisible(true)} />
            <ClientForm
